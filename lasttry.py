...
with col_radar:
    ...
    if selected_player != '-- Cluster Averages Only --':
        ...
        fig_radar_interactive.add_trace(go.Scatterpolar(
            ...
            line=dict(color = cluster_avg['color'], width=3),
            opacity=0.4,
        ...
        for cluster_num, cluster_data in cluster_averages.items():
            fig_radar_interactive.add_trace(go.Scatterpolar(
                ...
                hoveron='fills', # Makes the entire shape reactive to the mouse
                hovertemplate=f"<b>Cluster {cluster_num}</b><br>Value: %{{r:.2f}}<extra></extra>" # Shows cluster name on hover
            ))
